...
# 2. Environment variables — Molmo data in shared hf_cache, HF cache for models/datasets
export MOLMO_DATA_DIR=/data/hf_cache/molmo
export HF_HOME=/data/user_data/${USER}/.hf_cache
export HF_HUB_CACHE=/data/hf_cache/hub
export HF_DATASETS_CACHE=/data/hf_cache/datasets
export HF_TOKEN=   # Fill in for higher rate limits: https://huggingface.co/settings/tokens

# 3. Create directories (in case they don't exist)
mkdir -p "$MOLMO_DATA_DIR"
mkdir -p "$HF_HOME"

# 4. Go to molmo repo (SLURM_SUBMIT_DIR when sbatch; else script dir)
